        ...
        var r = function () {},
          ...
          h = function (c, h, f) {
            var v = n(),
              ...
              b = o(g),
              C = h || e;
            if (('function' == typeof C && (C = C()), $(this).hasClass('open') || $(this).hasClass('active')))
              return c.preventDefault(), d(), void (I = u.scrollTop);
            function w(e) {
              var a = $(e.target).parents().andSelf();
              a.is(T) || a.is(t) || a.is('.charts-popup-tab-headers, .charts-popup-itemheader') || d();
            }
            function x(t) {
            ...
            i.addClass && T.addClass(i.addClass), i.zIndex && T.css('z-index', i.zIndex);
            var k = T;
              ...
              $.each(C, function (e) {
                !(function e(s, n, o) {
                  if (s instanceof p.TabGroup) {
                    ...
                    var c = $('<div class="charts-popup-tab-group"></div>').appendTo(o),
                      l = $('<div class="charts-popup-tab-headers"></div>').appendTo(c),
                      r = null;
                    $.each(s.tabs || [], function (t, a) {
                      if (a.items && a.items.length) {
                        var n = $('<div class="charts-popup-tab"></div>').hide().appendTo(c);
                        $.each(a.items, function () {
                          e(this, void 0, n);
                        });
                        var o = $('<span class="charts-popup-tab-header">')
                          .append($('<a href="javascript://" class="charts-popup-tab-header-label">').text(a.name))
                          .appendTo(l);
                        o.on('click', function (t) {
                          o.is('.active') ||
                            ...
                            n.show(),
                            t && t.preventDefault(),
                            'function' == typeof s.onChange && s.onChange.call(s, a.name));
                        }),
                          (r && !a.active) ||
                            ((r = o),
                            l.find('.charts-popup-tab-header.active').removeClass('active'),
                    ...
                    var d = $('<div class="charts-popup-group"></div>').appendTo(o);
                    if (s.title) {
                      ...
                      s.collapsible &&
                        ...
                        u.on('click', function () {
                          d.toggleClass('collapsed'),
                            'function' == typeof s.onChange && s.onChange(d.hasClass('collapsed')),
                            T.height() === parseInt(T.css('max-height'))
                              ? T.addClass('popup-menu-scroll-y')
                              : T.height() < parseInt(T.css('max-height')) && T.removeClass('popup-menu-scroll-y');
                        })),
                        d.append(u);
                    }
                    ...
                    o.append($('<div class="charts-popup-itemheader"></div>').text(s.title));
                  else if (s.separator) {
                    ...
                    if (
                      ...
                      var v = s.action,
                        g = function (t) {
                          ...
                          e.is(x) ||
                            (v.apply(h, arguments),
                            !s.url && t && 'function' == typeof t.preventDefault && t.preventDefault());
                        };
                        ...
                        s.icon.offset &&
                          m.css(
                            ...
                            'string' == typeof s.icon.offset
                              ? s.icon.offset
                              : s.icon.offset.x + 'px ' + s.icon.offset.y + 'px',
                          ),
                        (f = $('<span class="title"></span>').appendTo(h));
                    } else
                      !0 === i.svg && s.svg
                        ? (i.wrapIcon
                            ? h.append($('<span class="icon-wrap">').addClass(s.iconClass).append(s.svg))
                            : h.append(s.svg),
                          (f = $('<span class="title"></span>').appendTo(h)))
                        : s.iconClass
                        ? (h.append($('<span class="icon"></span>').addClass(s.iconClass)),
                          (f = $('<span class="title"></span>').appendTo(h)))
                        : (f = $('<span class="title-expanded"></span>').appendTo(h));
                    s.html ? f.html(s.html) : f.text(TradingView.clean(s.title, !0) || '');
                    var b = $('<span class="shortcut"></span>').appendTo(h);
                    if ((s.shortcut && b.text(s.shortcut.keys), 'function' == typeof s.deleteAction)) {
                    ...
                    if (
                      ...
                      if ('function' == typeof s.counter) {
                        $('<span class="counter"></span>').html(s.counter()).appendTo(h);
                      } else {
                    ...
                    s.name && h.attr('data-name', s.name), o.append(h), t.data('popup-menu', o);
                  }
                })(this, e, k);
              }),
            ...
            var M = g.documentElement.clientWidth,
              ...
              E = t.offset(),
              I = $(m).scrollTop() || 0;
            (E.top -= I), (E.top = Math.round(E.top)), (E.left = Math.round(E.left));
            var S = T.outerWidth(),
              z = T.outerHeight(),
              ...
              P = z - T.height(),
              R = 'down';
            i.direction && (R = 'function' == typeof i.direction ? i.direction() : i.direction);
            var W = !!i.reverse;
            if ('down' === R) {
            ...
            switch (R) {
              ...
              case 'up':
                'down' === R
                  ...
                  W
                    ? T.css('left', Math.max(E.left + H + L - S, B) + 'px').css('right', 'auto')
              ...
              case 'left':
                (G = Math.max(G, 4)),
                  'right' === R
                    ? T.css('left', Math.floor(E.left + L + G) + 'px').css('right', 'auto')
                    : T.css('left', Math.floor(Math.max(E.left - S - G, B)) + 'px').css('right', 'auto'),
                  W
                    ? T.css('top', Math.floor(Math.max(E.top + H + A - z, B)) + 'px')
            ...
            var N = T.offset();
            T.css({ position: 'fixed', left: N.left - $(g).scrollLeft(), right: 'auto' }),
